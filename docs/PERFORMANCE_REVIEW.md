...
| # | Item | Risk | Expected impact |
|---|------|------|-----------------|
| 1 | Per-subscriber CBOR re-encoding of every broadcast frame | 🟡 | -30..60% server CPU on fanout, -20..40% p99 broadcast latency |
| 2 | `RECONNECT_INTERVAL = 60s` forces a full TCP+gRPC handshake every minute, per CLI | 🟢 | Removes a periodic 50–500 ms stall from the input path; lower server churn |
| 3 | `Vec<u8>` everywhere on the encrypt → gRPC → broadcast → WS hot path | 🟡 | -1 alloc + -1 memcpy per chunk per subscriber |
| 4 | `subscribe_chunks` clones the entire `Vec<Bytes>` buffer under the lock | 🟡 | Removes O(N·M) memcpy on subscriber join; smaller lock hold time |
| 5 | `RwLock<HashMap<Sid, State>>` is a write-heavy global; switch to per-shell locks | 🟡 | Reduces write contention; better tail latency under N shells × M viewers |
| 6 | Argon2 KDF runs on the WS task for every browser connect | 🟢 | Frees the websocket task; -50..200 ms time-to-first-frame on cold connects (frontend, also relevant if mirrored server-side) |
| 7 | `prev_char_boundary` is O(N) per chunk send | 🟢 | Removes a scan that grows with chunk size on every PTY read |
| 8 | TraceLayer + per-message info_span at high QPS | 🟢 | -5..15% CPU under load with `RUST_LOG=info` |
| 9 | `subscribe_chunks` polling loop wakes too often (one task per shell per browser) | 🟡 | Lower memory + scheduler pressure with many viewers |
| 10 | Redis pipe + 5 s sync interval for sequence numbers; consider event-driven sync | 🟡 | Lower median input-to-ack latency in lossy networks |
...
### 1.1 [🟢] Drop the 60 s forced reconnect on the CLI
...
### 1.2 [🟡] Re-encode CBOR once per broadcast, not once per subscriber
...
### 1.3 [🟢] Per-message `info_span` and full TraceLayer at INFO
...
### 1.4 [🟡] Bytes vs Vec<u8> on the hot path
**Where**:
...
### 1.6 [🟡] One spawned task per `Subscribe`, per shell, per browser
...
### 2.1 [🟡] `RwLock<HashMap<Sid, State>>` is a write-heavy global
...
### 2.2 [🟡] `subscribe_chunks` clones the entire chunk vec
...
### 2.3 [🟢] `BroadcastStream` capacity vs subscriber kill
...
### 2.4 [🟡] Move `Vec<(Sid, WsWinsize)>` watch payload to `Arc<...>`
...
### 2.5 [🟡] `update_user` on cursor moves broadcasts to everyone
...
### 3.1 [🟡] `Session::snapshot` zstd-compresses every 20 s
...
### 3.2 [🟢] `SerializedShell::data: Vec<Vec<u8>>` does an extra alloc per chunk
...
### 3.3 [🟢] `IdCounter::get_current_values` and `set_current_values` are unaccounted hotspots
...
### 3.4 [🟡] Rolling buffer size: 2 MiB on server, 8–12 MiB on client
**Where**:
...
### 3.5 [🟡] `last_accessed` uses `Mutex<Instant>` per session
...
### 3.6 [🟢] `tower_http::trace::TraceLayer` per-message allocations
See item 1.3.
...
### 4.1 [🟡] Ownership cache TTL of 2 s + Redis pipe on cache miss
...
### 4.2 [🟢] `set_options` with PX expiration on every snapshot write
...
### 4.3 [🟡] `proxy_redirect` does message-by-message translation
...
### 5.4 [🟡] AES-256-GCM: every chunk allocates `Vec` for nonce + ciphertext
...
### 6.1 [🔴] CBOR vs a length-prefixed compact framing
...
### 6.2 [🔴] gRPC vs raw QUIC / WebTransport for the CLI ↔ server link
...
1. ☐ Lower zstd level from 15 → 3 in `snapshot.rs:73` (1 line, ~3–5× faster snapshot CPU).
2. ☐ Replace `prev_char_boundary` with O(1) version (`runner.rs:143`).
3. ☐ `last_accessed: AtomicU64` instead of `Mutex<Instant>` (`session.rs:57`).
4. ☐ `watch::Sender<Arc<Vec<...>>>` for shells source (`session.rs:60`).
5. ☐ TraceLayer at DEBUG by default; INFO on errors only (`listen.rs:29,45`).
6. ☐ Remove or raise `RECONNECT_INTERVAL` (`controller.rs:26`); document in CHANGELOG.
7. ☐ Add `SSHX_SHELL_STORED_BYTES` env var (`session.rs:26`).
8. ☐ Bump `broadcast::channel` capacity from 256 → 1024 if logs show lag-kills (`session.rs:116`).

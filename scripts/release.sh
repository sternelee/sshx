...
cargo build -p sshx --release --target x86_64-unknown-linux-musl
...
cargo build -p sshx --release --target aarch64-unknown-linux-musl
...
cargo build -p sshx --release --target arm-unknown-linux-musleabihf
...
cargo build -p sshx --release --target armv7-unknown-linux-musleabihf

# x86_64-apple-darwin: for macOS on Intel
SDKROOT=$(xcrun --show-sdk-path) \
MACOSX_DEPLOYMENT_TARGET=$(xcrun --show-sdk-platform-version) \
cargo build -p sshx --release --target x86_64-apple-darwin

# aarch64-apple-darwin: for macOS on Apple Silicon
cargo build -p sshx --release --target aarch64-apple-darwin
